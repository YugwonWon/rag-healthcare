...
# ── 정서·관계 위험 신호 (전문가 검토 2026-06 추가) ──
# 가족 갈등·외로움·우울 등을 위험으로 인식해, 즉시 조언 X / 공감 + 탐색 우선.
EMOTIONAL_RISK_KEYWORDS = [
    # 가족 갈등 / 소외
    "구박", "외롭", "외로워", "쓸쓸", "혼자야", "혼자가",
    "가족이 싫", "가족이 미워", "사이가 안", "사이가 좋지", "사이가 나쁘",
    "말도 안 해", "말을 안 해", "말도 안해", "말을 안해",
    "보러 오지", "찾아오지", "전화도 안 ", "전화도 안해",
    "버림받", "버렸어", "기댈 데가 없", "의지할 데가 없",
    # 우울·자살 신호 (EMERGENCY와 일부 중복은 의도적)
    "죽고 싶", "살기 싫", "살 의미", "사는 의미", "살아서 뭐",
    "우울", "슬프", "눈물이 나", "울고만",
]


def detect_emotional_risk(message: str) -> bool:
    """정서·관계 위험 신호를 단순 키워드 매칭으로 감지."""
    if not message:
        return False
    return any(kw in message for kw in EMOTIONAL_RISK_KEYWORDS)
