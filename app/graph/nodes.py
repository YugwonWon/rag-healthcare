...
# ============================================================
# 노드 6-B: 대화 종료 (진행자가 마무리 요청)
# ============================================================

def end_conversation_node(state: ConversationState) -> dict:
    """대화를 자연스럽게 마무리하고 종료 신호를 반환한다.

    LLM·검색 없이 고정 마무리 멘트를 사용해 빠르고 안정적으로 끝낸다.
    `conversation_ended=True`로 프론트가 핸즈프리(VAD)를 멈추도록 한다.
    """
    nickname = state.get("nickname", "")
    logger.info(f"👋 대화 종료 | nickname={nickname}")
    return {
