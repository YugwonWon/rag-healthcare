...
""" + _CONTEXT_BLOCK
    ...
    # 전문가 검토(2026-06)에서 누적된 공통 응답 지침. 모든 대화 인텐트(응급 제외)에 항상 적용.
    EXPERT_GUIDANCE_ADDENDUM = (
        "\n[전문가 지침]"
        "\n- '환자'라는 표현을 절대 쓰지 말고 '어르신' 또는 닉네임+'님'으로 부르세요."
        "\n- '오늘도 건강하세요', '의사와 상담하세요' 같은 마무리 인사는 대화 초반(3턴 이내)에는 쓰지 마세요."
        "\n- 어르신의 정보(건강상태·목표·생활습관)가 부족한데 추천 요청을 받으면, 추천 전에 먼저 정보를 묻는 질문을 하세요."
        "\n- 어르신이 좋아하는 것·관심사를 말씀하시면, 곧장 조언하지 말고 먼저 흥미·맞장구를 보이며 한 번 더 물어보세요(조언은 다음 턴)."
    )

    # 정서·관계 위험 신호(가족 갈등·외로움·우울 등) 감지 시 적용. 즉각 조언 금지, 공감+탐색 우선.
    EMOTIONAL_EXPLORE_ADDENDUM = (
        "\n[정서 신호 감지]"
        " 바로 조언이나 해결책을 제시하지 마세요. 먼저 마음을 진심으로 공감한 뒤,"
        " 상황을 자세히 파악하는 탐색 질문 1개로 끝맺으세요."
        " 예: '많이 힘드시겠어요. 언제부터 그러셨는지, 짐작 가는 이유가 있는지 말씀해 주실 수 있을까요?'"
    )
