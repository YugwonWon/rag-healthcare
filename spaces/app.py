...
# ==========================================
# 관리자 — 대화 기록 조회/CSV 다운로드 (교수/연구자용)
# ==========================================

async def admin_login(password: str):
    """관리자 비밀번호 검증. 성공 시 조회/다운로드 영역을 노출한다."""
    ...
    async with httpx.AsyncClient(timeout=API_TIMEOUT, headers=headers) as client:
        try:
            ...
            if resp.status_code == 200:
                return "✅ 로그인 성공. 아래에서 대화 기록을 조회하거나 다운로드하세요.", gr.update(visible=True), password
...
with gr.Blocks(title="치매노인 맞춤형 헬스케어 챗봇") as demo:
    ...
    with gr.Tabs() as tabs:
        ...
        # 관리자 탭 (교수/연구자용 — 대화 기록 조회/CSV 다운로드)
        with gr.TabItem("🔐 관리자"):
            gr.Markdown(
                """
                ### 🔐 관리자 — 대화 기록 조회/다운로드
                연구 책임자/교수용 화면입니다. 비밀번호로 로그인한 뒤 전체 대화 기록을
                웹에서 표로 보거나 CSV로 내려받을 수 있습니다.
                """,
                elem_classes=["info-box"]
            )
            admin_pw = gr.Textbox(
